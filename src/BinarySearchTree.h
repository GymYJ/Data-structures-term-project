...
/**
*	@brief	Tree를 구성하는 Node (node data, left 포인터, right 포인터)
*/
template<typename T>
struct Node
{
	T data; ///< 현 노드의 데이터.
	Node* left; ///< 현 노드보다 작은 값을 가진 노드
	Node* right; ///< 현 노드보다 큰 값을 가진 노드
};
...
public:
	...
	*/
	...
	/**
	*	@brief	바이너리서치트리 간에 딥카피를 위한 = 연산자 오버로딩
	*	@param	tree	해당 클래스와 연산될 피연산자 클래스
	*/
...
void Replace(Node<T> *&root, T& item)
{
	if (root == NULL)						// root가 NULL인 경우 found는 false 
		...
		Replace(root->left, item);	// 왼쪽 노드로 가서 retrieve 함수 호출
	else if (item > root->data)				// 찾고자 하는 아이템값이 root값보다 클 때
		Replace(root->right, item);	// 오른쪽 노드로 가서 retrieve 함수 호출
